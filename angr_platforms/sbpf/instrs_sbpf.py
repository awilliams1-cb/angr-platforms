...
"""
Collections of sBPF instructions.

Forked from eBPF with the addition of jlt, jle, jslt, jsle conditional jumps.
"""
...
class SBPFInstruction(Instruction, abc.ABC):
    """Base Instruction for sBPF"""
...
class ArithmeticOrJumpInstruction(SBPFInstruction, abc.ABC):
...
class ALUInstruction(WithDestReg, ArithmeticOrJumpInstruction):
    ...
    def commit_result(self: ALUInstructionProtocol, res: VexValue) -> None:
        if self.size != REGISTER_TYPE:
            # ALU32: truncate result to 32 bits, then zero-extend to 64
            res = res.cast_to(self.size).widen_unsigned(REGISTER_TYPE)
...
class LoadOrStoreInstruction(WithDestReg, SBPFInstruction, abc.ABC):
...
class ImmediateSource(WithImmediate):
    ...
    @property
    def src_value(self: ALUInstructionWithImmediateProtocol) -> VexValue:
        # Always produce a 64-bit immediate to match register width.
        # ALU32/Jump32 class sets self.size to int_32, but sBPF registers
        # are always 64-bit, so VEX operations need matching types.
        return self.constant(self.immediate, REGISTER_TYPE)
...
class FetchSource:
    ...
    def fetch_operands(self: Any) -> Tuple[VexValue, ...]:
        return super().fetch_operands() + (self.src_value,)  # type: ignore
...
class FetchDestination(WithDestReg):
    ...
    def fetch_operands(self: Any) -> Tuple[VexValue, ...]:
        return super().fetch_operands() + (self.get(self.dest_reg, REGISTER_TYPE),)  # type: ignore
...
class FetchPC:
    ...
    def fetch_operands(self: Any) -> Tuple[VexValue, ...]:
        return super().fetch_operands() + (self.get("ip", REGISTER_TYPE),)  # type: ignore
...
class LshOp(FetchDestination, FetchSource):
    ...
    def compute_result(self, src, dst):
        # VEX shift ops require an 8-bit shift amount
        return dst << src.cast_to(Type.int_8)
...
class ArshOp(FetchDestination, FetchSource):
    ...
    def compute_result(self, src, dst):
        return dst.sar(src.cast_to(Type.int_8))
...
class ConditionalJumpOp(WithOffset, FetchDestination, FetchSource, FetchPC):
    ...
    def compute_result(self: SourcedConditionalJumpInstructionProtocol, pc, src, dst):
        # Jump32 produces a 32-bit immediate but registers are 64-bit;
        # widen the narrower operand so VEX comparison ops are well-typed.
        if src.ty != dst.ty:
            src = src.widen_unsigned(dst.ty)
        cond = self.condition(src, dst)
        self.jump(cond, pc + self.offset, JumpKind.Boring)
...
# --- NEW: Missing jump operations for sBPF ---
